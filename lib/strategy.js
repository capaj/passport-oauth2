...
const log = require('debug')('passport-centrify-oauth2')
 ...
 * Options:
 *
 *   - `name`              name used by passport.js to identify this strategy
...
class OAuth2Strategy {
  constructor(options, verify) {
    ...
    this.name = options.name || 'centrify-oauth2'
    this._verify = verify
  ...
  authenticate(req, options = {}) {
    ...
    if (req.query && req.query.code) {
      ...
      const getAuthorizeUrl = (params) => {
        ...
        parsed.query['client_secret'] = self._oauth2._clientSecret
        if (options.debug) {
          parsed.query['debug'] = true
        }
        if (!params.scope && options.scope) {
          parsed.query['scope'] = options.scope
        }
        log('parsed: ', parsed)
      ...
      if (state) {
        ...
        const location = getAuthorizeUrl(params)
